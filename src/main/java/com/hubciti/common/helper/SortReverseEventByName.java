...
public class SortReverseEventByName implements Comparator<EventDetails> {

	/**
	 * Logger instance.
	 */
	private static final Logger LOG = LoggerFactory.getLogger(SortReverseEventByName.class.getName());
		...
		try {
			...
			if (o1.getIsOnGoing() == 1 && o2.getIsOnGoing() == 0) {
				...
				if (o1.getIsOnGoing() == 0 && o2.getIsOnGoing() == 1) {
					...
					iCategory = strEventName2.compareTo(strEventName1);
				}
		} catch (Exception e) {
			LOG.error("ParseException in SortReverseEventByName : Exception : " + e.getMessage());
		}
		return iCategory;
	}
}
