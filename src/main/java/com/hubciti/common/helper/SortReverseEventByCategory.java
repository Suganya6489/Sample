...
public class SortReverseEventByCategory implements Comparator<EventDetails> {

	/**
	 * Logger instance.
	 */
	private static final Logger LOG = LoggerFactory.getLogger(SortReverseEventByCategory.class.getName());
	...
	public int compare(EventDetails o1, EventDetails o2) {
		...
		try {
			...
			if (o1.getIsOnGoing() == 1 && o2.getIsOnGoing() == 0) {
				...
				iCategory = strEventCategory2.compareTo(strEventCategory1);
			}
		} catch (Exception e) {
			LOG.error("ParseException in SortReverseEventByCategory : Exception : " + e.getMessage());
		}
		return iCategory;
	}
}
